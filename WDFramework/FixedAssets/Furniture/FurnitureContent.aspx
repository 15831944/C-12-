<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FurnitureContent.aspx.cs" Inherits="WDFramework.FixedAssets.Furniture.FurnitureContent1" %>
...
<body oncontextmenu='return false'><%--取消鼠标右键的点击--%>
   ...
   <x:Panel ID="Panel1" runat="server"  ShowBorder="True"  EnableCollapse="true"
        ...
        <Items>
            <x:Grid ID="Grid_Furniture" runat="server" BoxFlex="1" ShowBorder="true" ShowHeader="false"
      EnableRowNumberPaging="true" EnableTextSelection="true" EnableCheckBoxSelect="false"
         DataKeyNames="FurnitureID" AllowSorting="true" SortColumnIndex="0"
        ...
        <Toolbars>
                            <x:Toolbar ID="Toolbar_top" runat="server" >
                                ...
                                <x:DropDownList ID="ddl_search" ShowLabel="false" AutoPostBack="true" runat="server" EnableEdit="false" OnSelectedIndexChanged="ddl_search_SelectedIndexChanged">
                                   ...
                                   <x:ListItem Text="家具名称" Value="1" />
                                   <x:ListItem Text="购买人" Value="2" />
                                   <x:ListItem Text="购买时间" Value="3" />
                                   <x:ListItem Text="价格" Value="4" />
                                   <x:ListItem Text="使用人" Value="5" />
                                   <x:ListItem Text="所属机构" Value="6" />

                                   <x:ListItem Text="存放地点" Value="7" />
                                   <x:ListItem Text="资产编号" Value="8" />
                                </x:DropDownList> 
                                    ...
                                    <%--<x:TwinTriggerBox runat="server" EmptyText="输入要搜索的关键词" ShowLabel="false" ID="ttbSearch" ShowTrigger1="false"
                                     ...
                                     <x:Button ID="btnAddEquipment" runat="server" EnablePostBack="false"  Icon="Add" Text="新增家具信息">
                                     </x:Button>
       ...
       <%--DataKeyNames这是数据库唯一标识--%>
         <Columns>
            ...
            <x:BoundField DataField="FurnitureName" SortField="EquipmentName" Width="150px" HeaderText="家具名称" />
            ...
            <x:TemplateField Width="80px" HeaderText="盘亏设备">
                <ItemTemplate>
                    <asp:Label ID="Label2" runat="server" Text='<%# Convert.ToString(DataBinder.Eval(Container.DataItem, "CategoryName")) == "盘亏设备" ? "是" : "否" %>'></asp:Label>
                </ItemTemplate>
            </x:TemplateField>
            <x:TemplateField Width="80px" HeaderText="保密级别">
                <ItemTemplate>
                    <asp:Label ID="Label11" runat="server" Text='<%# ChangeSecrecyLevel(Convert.ToInt32(DataBinder.Eval(Container.DataItem, "SecrecyLevel"))) %>'></asp:Label>
                </ItemTemplate>
            </x:TemplateField>     
          ...
          <x:Window ID="Window_add" Popup="false" EnableIFrame="true" runat="server" 
                          EnableMaximize="false" EnableMinimize="false"  EnableResize="false" Height="375px" Width="450px">
        </x:Window>
